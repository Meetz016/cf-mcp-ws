...
// .wrangler/tmp/bundle-2GXPWj/checked-fetch.js
...
// src/repository/publisher/publisher.repository.ts
async function publish(env, data) {
  try {
    const { publisher_id, stock_name, stock_symbol, stock_price } = data;
    const dbResult = await env.DB.prepare("SELECT * FROM publisher WHERE publisher_id = ?").bind(publisher_id).all();
    let finalPublisherId = publisher_id;
    if (dbResult.results.length === 0) {
      finalPublisherId = crypto.randomUUID();
      await env.DB.prepare("INSERT INTO publisher (publisher_id) VALUES (?)").bind(finalPublisherId).run();
    }
    const stockId = crypto.randomUUID();
    await env.DB.prepare(`
                INSERT INTO stock (stock_id, stock_name, stock_symbol, stock_price, publisher_id)
                VALUES (?, ?, ?, ?, ?)
            `).bind(stockId, stock_name, stock_symbol, stock_price, finalPublisherId).run();
    return {
      success: true,
      message: dbResult.results.length === 0 ? "Publisher didn't exist. Created new publisher and published stock." : "Stock published successfully.",
      data: {
...
// src/repository/stock/stock.repository.ts
async function searchStock(env, stock_name) {
  console.log("Looking for stock:", stock_name);
  try {
    const stock = await env.DB.prepare("SELECT * FROM stock WHERE UPPER(stock_name) = UPPER(?)").bind(stock_name).all();
...
__name(searchStock, "searchStock");
async function getAllStocks(env) {
  try {
...
__name(getAllStocks, "getAllStocks");

// src/repository/subscriber/subscriber.repository.ts
async function searchSubscriber(env, subscriber_id) {
  try {
    ...
    const subscriber = await env.DB.prepare("SELECT * FROM subscriber WHERE subscriber_id = ?").bind(subscriber_id).all();
...
__name(searchSubscriber, "searchSubscriber");
async function addSubscriber(env) {
  try {
    const subscriber_id = crypto.randomUUID();
    await env.DB.prepare("INSERT INTO subscriber (subscriber_id) VALUES (?)").bind(subscriber_id).run();
    return {
      success: true,
...
// src/repository/subscriptions/subscriptions.repository.ts
async function addSubscription(env, stock_name, subscriber_id) {
  try {
    ...
    ).bind(stock_id, subscriber_id).all();
    ...
    ).bind(subscription_id, stock_id, subscriber_id).run();
    return {
      success: true,
...
// src/durable_objects/connections.ts
var MCPConnectionsDO = class {
  static {
    __name(this, "MCPConnectionsDO");
  }
   ...
   */
  async handlePublisherMessage(server, message) {
    ...
    if (message.isNewStock && message.id && message.payload.price) {
      ...
      if (publishResult.success) {
        ...
        this.topics.set(stock.toLowerCase(), /* @__PURE__ */ new Set());
      }
      ...
      if (!stockExists) {
        ...
        if (!isExists.success) {
          ...
          this.topics.set(stockLower, /* @__PURE__ */ new Set());
   ...
   */
  async handleSubscriberMessage(server, message, clientId) {
...
// .wrangler/tmp/bundle-2GXPWj/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-2GXPWj/middleware-loader.entry.ts
