...
// .wrangler/tmp/bundle-HdOfCi/checked-fetch.js
...
async function searchStock(env, stock_name) {
  try {
    ...
    const stock = await env.DB.prepare("SELECT * FROM stock WHERE stock_name = ?").bind(stock_name.toUpperCase()).all();
    console.log(stock);
  ...
  async handlePublisherMessage(server, message) {
    ...
    if (message.isNewStock && message.id && message.payload.price) {
      ...
      if (publishResult.success) {
        ...
        this.topics.set(stock.toLocaleLowerCase(), /* @__PURE__ */ new Set());
      }
      ...
      if (!this.topics.get(stock)) {
        ...
        if (!isExists.success) {
          ...
          this.topics.set(stock, /* @__PURE__ */ new Set());
...
// .wrangler/tmp/bundle-HdOfCi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HdOfCi/middleware-loader.entry.ts
